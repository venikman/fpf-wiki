...
U.MethodDescription: MD.DecisionDesktop.MVP.v1    (controlled narrative, ~F3)
  Objective:
    ...
    (Deliverable described in Strategic Framework.)
  ...
  Inputs (pre-work):
    - Draft PlainView by DecisionOwnerRole (10–15 min).
  ...
  Output artifacts (end of Workshop Work):
    ...
    C) Review Cadence (weekly meeting locked: day/time/duration/attendees)

  Procedure (Workshop Work, 75 min):
    Step 1 (0–10 min): Finalize PlainView (silent read, clarify, finalize).
    Step 2 (10–30 min): Premortem: generate failures (skill then luck), vote top 5, assign p + early signals.
    Step 3 (30–50 min): Backcast: generate success drivers (skill then luck), vote top 5, assign p + early signals.
    Step 4 (50–65 min): Build Exploration Table: merge scenarios, add owners, review cadences, and mitigation/amplifier notes.
    Step 5 (65–75 min): Schedule Review + Close: lock weekly review; DecisionOwnerRole commits to pre-work.

  Exclusions (MVP discipline):
    - No “Dr. Evil”/adversarial persona exercise in Week 1 workshop.
    - Precommitments and hedges may be drafted only if time remains; otherwise defer to later. (Consistent with "start with MVP" guidance.)

  Acceptance (what “done” means):
    - A single page exists with PlainView + Exploration Table + Review cadence.
    - Weekly review can update the page in <5 min after meeting.
...
Conformance checklist (minimal, spec-like):
  - Every Work instance MUST be recorded as U.Work and MUST reference the MethodDescription it executes
    (separating recipe from occurrence).
  - Every planned meeting MUST live in U.WorkPlan; do not write "what happened" into the plan.
  - Success Criteria MUST be observable and time-bounded; Review Date MUST be present.
  - Each top scenario row MUST have an owner and review cadence.
  - Each weekly review MUST limit decisions to max 1 change.
  - HF-loop: after 2 enactments, capture friction points and revise MD/WP for cognitive load.
  - Proxy-audit: if a metric is a proxy for the true objective, declare it as proxy and re-audit monthly.
