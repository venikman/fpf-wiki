// Read-only in-memory storage for FPF artifacts
...
// Static FPF artifact data
const fpfArtifacts: Artifact[] = [
  {
    id: "a1",
    patternId: "A.1",
...
export class MemStorage implements IStorage {
  ...
  async getArtifactByPatternId(patternId: string): Promise<Artifact | undefined> {
    return Array.from(this.artifacts.values()).find(
      (artifact) => artifact.patternId === patternId
    );
  }
